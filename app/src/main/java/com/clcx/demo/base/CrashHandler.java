...
public class CrashHandler implements UncaughtExceptionHandler {
	// 保存文件夹名字
	private static final String CRASH_FILE_PATH = "TongyongCrash";
	...
	/*
	 * 出现未捕获的异常时, 会自动回调该方法 (non-Javadoc)
	 * 
	 * @see
	...
	public void uncaughtException(Thread thread, Throwable ex) {
		/*
		 * 调用 handleException() 方法处理该线程 如果返回 true 说明处理成功, 如果返回 false
		 * 则调用默认的异常处理器来处理 一般情况下该方法都会成功处理
		 */
	...
	private boolean handleException(Throwable ex) {
		...
		/*
		 * 使用Toast来显示异常信息, 由于在主线程会阻塞, 不能实时出现 Toast 信息, 这里我们在子线程中处理 Toast 信息
		 */
		new Thread() {
			@Override
			public void run() {
				Looper.prepare();
				Toast.makeText(mContext, "很抱歉,程序出现异常,即将退出,请与开发者联系.",
